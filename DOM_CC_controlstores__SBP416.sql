...
/* export as "DOM_CC_controlstores.csv" */

WITH CONTROL_STORE_DATA AS (
SELECT
   QSTN_ID
  ,CONTROL_STORE_NUM
  ,DOM_INSTALL_DT
...
JOIN (SELECT DISTINCT CONTROL_STORE_NUM, DOM_INSTALL_DT FROM DEPT_CUST_INS.TN_DOM_TEST_STORES) dom
  ON sr.STORE_NUM = dom.CONTROL_STORE_NUM
...
SELECT
  ...
  ,QSTN_ID
  ,MOBILE_ORD_PAY_IND
  ,COUNT(DISTINCT CONTROL_STORE_NUM) AS STORE_CNT_CHECK
  ,SUM(CASE WHEN DAYS_FROM_INSTALL BETWEEN -56 and -8 THEN TB_COUNT ELSE 0 END) AS TB_COUNT_PRE --week 1-8 prior
  ,SUM(CASE WHEN DAYS_FROM_INSTALL BETWEEN 28 and 90 THEN TB_COUNT ELSE 0 END) AS TB_COUNT_POST --weeks 5-13 post
...
SELECT
  ...
  ,dom.CONTROL_STORE_NUM 
  ,csd.QSTN_ID
  ,csd.MOBILE_ORD_PAY_IND
  ,csd.DAYS_FROM_INSTALL
  ,csd.TB_COUNT
  ,csd.RSPNS_COUNT
  ,csd.PEAK_TB_COUNT
  ,csd.PEAK_RSPNS_COUNT
FROM DEPT_CUST_INS.TN_DOM_TEST_STORES dom

LEFT JOIN CONTROL_STORE_DATA csd
  ON dom.CONTROL_STORE_NUM = csd.CONTROL_STORE_NUM
    AND dom.DOM_INSTALL_DT = csd.DOM_INSTALL_DT
)
