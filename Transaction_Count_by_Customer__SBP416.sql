...
WITH GUID_LIST AS (
  SELECT
  sr.GUID_USER_ID as GUID
  ,SUM(CASE  WHEN sr.RSPNS_ID = '7' THEN 1 ELSE 0 END) AS TOTAL_TB
  ,COUNT(*) AS TOTAL_RSPNS
  ,ca.FSCL_YR_NUM
  ,ca.FSCL_PER_IN_YR_NUM
  ,TRUNC(sr.TRANS_DTM) AS SRVY_TRANS_DT
FROM APPOTHER.AFT_CV_SRVY_RSPNS sr
...
AND f.SVC_TRANS_CTGY = 'Redemption' and f.RVRS_IND = 0
--AND f.SALES_CHNL_CD = 'CO' 
AND f.CUST_CNTRY_CD = 'US'
AND f.TRANS_DT >= '01-DEC-17' AND f.TRANS_DT <= '31-DEC-17'
ORDER BY f.GUID_ID, f.TRANS_DT


select * from APPCA.F_SVC_FIN_TRANS a
inner join APPCA.D_SVC_TRANS_TYPE c 
  on a.SVC_TRANS_TYPE_KEY = c.SVC_TRANS_TYPE_KEY
WHERE a.GUID_ID IN ('0A431223-46E3-438C-B301-B9AEDE967A29','6454D29D-5CC3-4809-AD03-4FA2CBCCFF6B','8F959E6B-5B96-4FD9-B699-04CDE1475DC5')
AND c.SVC_INTRNL_RQST_NM = 'Redemption' 
AND a.TRANS_DT >= '01-DEC-17' AND a.TRANS_DT <= '31-DEC-17'
ORDER BY a.GUID_ID, a.TRANS_DT
