...
#merge together predicated and actual [prophet model]
...
#average across the week to calculated pred- to observed- residuals
...
#using library(mFilter)
#Hodrick-Prescott filter to remove cyclical component of time series
temp <- pcc[,.(ds,y)]
temp <- setorder(temp,ds)
temp.hp1 <- hpfilter(temp[,y], freq=12,type="frequency",drift=TRUE) #frequency, freq = cut-off frequency of HP filter. drift TRUE if drift in time series.
temp.hp2 <- hpfilter(temp[,y], freq=52,type="frequency",drift=TRUE) 
temp.hp3 <- hpfilter(temp[,y], freq=129600,type="lambda",drift=TRUE) #lambda, freq = smoothing parameter
...
#merge together predicated and actual [auto.arima]
...
#exponential smoothing is usually used for data with no trend or seasonal pattern... so...?
HWx_new_sea <- dshw(temp[,"y"], period1=7, period2=7*52, h=25) #double seasonality
HWx_new_exps <- dshw(temp[,"y"], period1=7, period2=7*52, h=25, beta=FALSE, gamma=FALSE) #double seasonality and exponential smoothing
...
#predictive modeling with covariates
#customer connection
...
#create lags for cc
ccwkdt[, lag_cc_lw := lapply(.SD, function(x) c(NA, x[-.N])), by="STORE_NUM", .SDcols="cc"] #1 week
ccwkdt[, lag_cc_lw2 := lapply(.SD, function(x) c(NA, x[-.N])), by="STORE_NUM", .SDcols="lag_cc_lw"] #2 weeks
ccwkdt[, lag_cc_ly := lapply(.SD, function(x) c(NA, x[-.N])), by=c("STORE_NUM","FSCL_WK_IN_YR_NUM"), .SDcols="cc"] #1 year
...
lm1 <- lm(cc ~ lag_cc_lw, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm1)
lm2 <- lm(cc ~ lag_cc_lw + lag_cc_ly, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm2)
lm3 <- lm(cc ~ hs + pt + tsd + lag_cc_lw, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm3)
lm4 <- lm(cc ~ hs + pt + tsd + lag_cc_lw + lag_cc_ly, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm4)
lm5 <- lm(cc ~ lag_cc_lw + lag_cc_lw2 + lag_cc_ly, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm5)
lm6 <- lm(cc ~ hs + pt + tsd + lag_cc_lw + lag_cc_lw2, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm6)
lm7 <- lm(cc ~ hs + pt + tsd + lag_cc_lw + lag_cc_lw2 + lag_cc_ly, data = ccwkdt[FSCL_YR_NUM==2018])
summary(lm7)
#compare the nested models
anova(lm1,lm2,lm3,lm4,lm5,lm6,lm7,test="Chisq")
...
acf(lm7$residuals,lag.max=30)

#test whether there is significant evidence for non-zero correlations at lags 1-30
Box.test(lm7$residuals, lag=30, type="Ljung-Box")

#ensure forecast errors are normally distributed with mean zero and constant variance
plot.ts(lm7$residuals,main="LM Residuals by Day")

#plot
plotForecastErrors(lm7$residuals)

#ts model using library("TTR")
temp <- pcc[,.(ds,y)]
temp <- setorder(temp,ds)
temp <- as.matrix(temp[,.(y)])
temptimeseries <- ts(temp, frequency=365, start=c(2015,244))
plot.ts(temptimeseries)
# temptimeseriesSMA365 <- TTR::SMA(temptimeseries,n=365)
...
# hw1fcast <- predict(hw1, n.ahead=21, prediction.interval=T)
# plot(hw1,hw1fcast,main="HoltWinters Seasonal")
#HW, non-seasonal
hw2 <- HoltWinters(temptimeseries, gamma=FALSE)
hw2fcast <- predict(hw2, n.ahead=21, prediction.interval=T)
...
hw3fcast <- predict(hw3, n.ahead=21, prediction.interval=T)
